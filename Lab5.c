...
int main(void){
  ...
  if(BSP_Button1_Input() == 0){ // run TExaS if Button1 is pressed
    ...
    TExaS_Init(GRADER, 1604);   // initialize the Lab 5 grader
...
/*  OS_File_Read(n,2,Buff);
	OS_File_Read(n,3,Buff);
	OS_File_Read(n,4,Buff);
	OS_File_Read(m,2,Buff);
	OS_File_Read(m,3,Buff);
	OS_File_Read(m,4,Buff);
	OS_File_Read(p,2,Buff);
	OS_File_Read(p,3,Buff);
	OS_File_Read(p,4,Buff);*/
